...
public class WindowsSymbolExtractor extends AbstractCompiler<SymbolExtractorSpec> {
  public WindowsSymbolExtractor(BuildOperationExecutor buildOperationExecutor, CommandLineToolInvocationWorker commandLineToolInvocationWorker, CommandLineToolContext invocationContext, WorkerLeaseService workerLeaseService) {
  ...
  private static class SymbolExtractorArgsTransformer implements ArgsTransformer<SymbolExtractorSpec> {
    ...
    public List<String> transform(SymbolExtractorSpec spec) {
        ...
        args.add("/OUT:" + spec.getSymbolFile().getAbsolutePath());
